...
function createAdvancedTeam(team) {
  return {
    name: team.name,
    logo: team.logo
  };
...
function handleQuarterFinalPick(matchId, teamIndex) {
  ...
  if (matchId === "qf1") {
    state.semiFinals[0].teams[1] = createAdvancedTeam(selectedTeam);
    clearWinner("sf1");
  ...
  if (matchId === "qf2") {
    state.semiFinals[1].teams[1] = createAdvancedTeam(selectedTeam);
    clearWinner("sf2");
...
function handleSemiFinalPick(matchId, teamIndex) {
  ...
  if (matchId === "sf1") {
    state.grandFinal[0].teams[0] = createAdvancedTeam(selectedTeam);
    clearWinner("gf1");
    state.champion = "TBD";
  }

  if (matchId === "sf2") {
    state.grandFinal[0].teams[1] = createAdvancedTeam(selectedTeam);
    clearWinner("gf1");
    state.champion = "TBD";
  }
